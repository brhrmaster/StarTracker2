...
void Control::main_menu() {
    ...
    _display.render_main(_last_state_changed || _last_substate_changed, _substate, Clock::get_LST(), 
                         _mount.is_tracking(), _mount.is_moving());
            
    if (_keypad.pushed(C_EXIT)) {
        if (_mount.is_tracking()) _mount.stop_tracking();
...
bool Control::find_in_catalogue(ControlSubState catalogue, int object, MountController::coord_t& coords, float& magnitude, float& size_a, float& size_b, char type[6]) {
    #ifdef DEBUG_CONTROL
        Serial.println(F("=============="));
        File root = SD.open("/");
        while (true) {
