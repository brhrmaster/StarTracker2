...
#define C_CALIBRATION           KP_KEY_SETUP
#define C_TIME					KP_KEY_5
#define C_BRIGHTNESS            KP_KEY_MUTE
...
class Control {
      ...
      public:
        ...
        enum State { MAIN, HELP, GOTO, CALIB, CATALOG, TIME, POSITION, BRIGHT };

        Control(MountController& mount, Clock& clock)
            : _mount(mount), _clock(clock) {}

        // initialize display, mount, clock and SD card
        void initialize();
    ...
    private:
        ...
        template <class T> void save(T value, uint16_t adress) {
            byte* p = (byte*)(void*)&value;
            for (uint32_t i = 0; i < sizeof(value); i++) EEPROM.write(adress++, *p++);
        }

        template <class T> void load(T& target, uint16_t adress, T lower, T upper, T default_value) {
            load(target, adress);
            if (target >= lower && target <= upper) return; // NaN returns false for all comp.
            target = default_value;
        }

        template <class T> void load(T& target, uint16_t adress) {
            byte* p = (byte*)(void*)&target;
            for (uint32_t i = 0; i < sizeof(target); i++) *p++ = EEPROM.read(adress++);
        }
        ...
        bool find_in_catalogue(ControlSubState catalogue, int object, MountController::coord_t& coords, float& magnitude, float& size_a, float& size_b, char type[5]);
