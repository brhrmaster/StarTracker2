...
void Display::initialize(int brightness) {
    ...
    _lcd.setCursor(0, 1); 
    _lcd.print(F("Press BACK for help")); 
...
void Display::render_main(bool refresh, ControlSubState phase, const DateTime& lst, bool tracking, bool moving) {
    ...
    _lcd.setCursor(12, 1); 
    if (tracking || moving) _lcd.print(F("/"));
    else _lcd.print(F(" "));
